...
void NNSutil(Point p, Node* head, Node* T, int K, int dim) {
	...
	if (dist < Q.top().first && dist != 0)
		...
		NNSutil(p, head, T->left, K, T->split);//T->split, (dim + 1) % k)
		NNSutil(p, head, T->right, K, T->split);
	
}

void NNSinterface(Point p, Node* head, Node* T, int K, int dim) {
	bool left = false;
	if (p.data[dim] < P[T->index].data[dim]) {
		NNSutil(p, head, T->left, K, T->split);
		left = true;
	}
	else NNSutil(p, head, T->right, K, T->split);
	//pruning condition
	double distdist = euclidianDistance(P[T->index].data, p.data);
	double bb = Q.top().first;
	if (distdist < bb && distdist != 0) {  //distdist < bb && distdist != 0
		if (left)
			NNSutil(p, head, T->right, K, T->split);
		else
			NNSutil(p, head, T->left, K, T->split);
...
int main(int argc, char *argv[])
{
	...
	for (int i = 0; i < N; i++) {
		NNSinterface(P[i], T, T, K, T->split);
		computeWeightedAvg();
	}
